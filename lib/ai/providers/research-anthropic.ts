...
import type { ResearchSynthesizer } from '../../research/worker'
import { createResearchSynthesizer, SYNTHESIS_MAX_TOKENS } from '../../research/providers/synthesis'
import type { ResearchCompletionRequest, ResearchCompletionResult } from '../../research/providers/types'
import type { ResearchCapability, ResearchProviderAdapter, ResearchProviderHealth } from '../../research/providers/types'
...
/** Provider-native search/fetch adapter for the canonical AI research boundary. */
export function createAnthropicResearchAdapter(options: AnthropicResearchProviderOptions = {}): ResearchProviderAdapter {
  const model = options.model ?? DEFAULT_RESEARCH_MODEL
  ...
  const capabilities: readonly ResearchCapability[] = ['web_search', 'source_citations', 'durable_source_fetch', 'structured_output', 'long_context']
  let sensor: ReturnType<typeof createAnthropicResearchProvider> | undefined
  const delegate = () => (sensor ??= createAnthropicResearchProvider({ client: client(), model }))

  return {
    id: 'anthropic', model, name: `anthropic:${model}`, capabilities,
    async checkHealth(): Promise<ResearchProviderHealth> {
      const apiKey = options.apiKey ?? process.env.ANTHROPIC_API_KEY
      return !options.client && !apiKey ? { usable: false, detail: 'ANTHROPIC_API_KEY is not set.' } : { usable: true }
    },
    search(query, searchOptions) { return delegate().search(query, searchOptions) },
    fetch(result) { return delegate().fetch(result) },
    complete(request) { return createAnthropicResearchCompletion({ client: options.client, model, onUsage: options.onUsage })(request) },
  }
}
