...
describe('Direction recommendation + decision mission-control contract', () => {
  ...
  it('keeps the mission-control states compact and evidence expandable', () => {
    ...
    expect(card).toMatch(/ACTING NOW/)
    expect(card).toMatch(/APPROVED · QUEUED/)
    expect(card).toMatch(/WAITING FOR YOU/)
    expect(card).toMatch(/COMPLETED/)
    expect(card).toMatch(/FAILED · NEEDS ATTENTION/)
