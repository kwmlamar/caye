...
export async function dryRunOutreachSelection(workspaceId: string, now = new Date(), limit = 40): Promise<OutreachDryRun> {
  ...
  const { data, error } = await db.from('outreach_leads').select('id,lead_email,business_name,stage,created_at,first_touch_sent_at,touches_sent,last_touch_sent_at,opted_out_at,disqualified_reason').eq('workspace_id', workspaceId).not('stage', 'in', '("won","lost","disqualified")').is('outreach_claim_token', null).order('created_at')
  ...
  const plans = rows.flatMap((row) => {
    ...
    const action = decideNextAction({ stage: row.stage, firstTouchSentAt: row.first_touch_sent_at, touchesSent: row.touches_sent, lastTouchSentAt: row.last_touch_sent_at, optedOutAt: row.opted_out_at, hasUnansweredReply: conversation?.last_sender_type === 'customer', disqualifyingSignal: row.disqualified_reason }, now)
