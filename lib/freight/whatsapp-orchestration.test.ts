...
import type { FreightReference } from './types'
import { ambiguousFreightQuestion, artifactVersion, bindFreightApproval, classifyFreightOwnerIntent, freightOwnerSummary, resolveFreightReferent, validateFreightApproval } from './whatsapp-orchestration'

// `reference` is the source of truth ({ kind, value }); a bare string is shorthand for a King
// Ocean dock receipt. `dockReceiptNumber` is set exactly the way detection.ts derives it -- only
// when the kind is dock_receipt -- so a TWINex fixture reproduces production's `null`.
function workflow(id: string, refOrDock: string | FreightReference, status: FreightWorkflowRecord['status'] = 'READY_FOR_APPROVAL'): FreightWorkflowRecord {
  const reference: FreightReference = typeof refOrDock === 'string' ? { kind: 'dock_receipt', value: refOrDock } : refOrDock
  const dock = reference.kind === 'dock_receipt' ? reference.value : null
  return {
    id, workspaceId: 'ods', conversationId: `conv-${id}`, requestMessageId: `msg-${id}`,
    request: { isFreightDocumentRequest: true, evidence: [], freightProvider: 'King Ocean', senderName: 'Nicole', senderEmail: 'nicole@example.test', reference, dockReceiptNumber: dock, shipmentReference: null, requestedDocument: 'invoice', consolidationMentioned: false, destination: null, commodities: [], requestedAt: '2026-08-29T12:00:00Z' },
    status,
    candidates: [{ evidence: { id: `receipt-${id}`, workspaceId: 'ods', source: 'artifact', vendor: 'Home Depot', purchaseDate: '2026-08-29', referenceNumbers: [reference.value], orderNumber: null, receiptNumber: `HD-${id}`, poNumber: null, lines: [], subtotal: 800, tax: 42.17, shipping: null, total: 842.17, currency: 'USD', purchaser: null, shippingAddress: null, filename: 'receipt.pdf', provenance: [] }, score: 100, confidence: 'HIGH', reasons: ['exact reference'] }],
...
describe('WhatsApp freight orchestration', () => {
  ...
  // TWINex is roughly half of ODS's freight and issues a warehouse number, not a dock receipt.
  // Every assertion below fails if the code reads the King-Ocean-only `dockReceiptNumber`
  // projection (null for TWINex) instead of `reference`.
  describe('TWINex warehouse-number requests (no dock receipt)', () => {
    const twinex = (id: string, value: string) => workflow(id, { kind: 'warehouse_number', value })

    it('names the warehouse number in owner-facing prose instead of dropping the identifier', () => {
      const summary = freightOwnerSummary(ref(twinex('t', '188052')))
      expect(summary).toContain('warehouse 188052')
      expect(summary).not.toContain('null')
      expect(summary).not.toContain('undefined')
    })

    it('resolves a referent when the owner quotes the warehouse number', () => {
      const result = resolveFreightReferent({
        workspaceId: 'ods',
        text: 'send the one for 188052',
        referents: [ref(twinex('t', '188052')), ref(workflow('k', 'DR-12345'))],
      })
      expect(result).toMatchObject({ kind: 'resolved', referent: { workflow: { id: 't' } } })
    })

    it('disambiguates by warehouse number rather than offering blank choices', () => {
      const result = resolveFreightReferent({
        workspaceId: 'ods',
        text: 'send it',
        referents: [ref(twinex('t1', '188052')), ref(twinex('t2', '188099'))],
      })
      expect(result.kind).toBe('ambiguous')
      if (result.kind !== 'ambiguous') return
      const question = ambiguousFreightQuestion(result.options)
      expect(question).toContain('188052')
      expect(question).toContain('188099')
    })
  })
