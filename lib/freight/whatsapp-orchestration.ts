...
import { freightReferenceLabel } from './types'
...
function referentText(r: FreightReferent): string {
  ...
  return normalized([r.providerLabel, req.freightProvider, req.senderName, req.senderEmail, req.reference?.value, req.dockReceiptNumber, req.shipmentReference].filter(Boolean).join(' '))
}
...
export function resolveFreightReferent(input: {
  ...
  const scored = candidates.map(referent => {
    ...
    // Keyed off `reference`, not the King-Ocean-only projection: an owner who types a TWINex
    // warehouse number must get the same exact-identifier confidence as a dock receipt.
    if (req.reference && t.includes(normalized(req.reference.value))) score += 8
...
export function freightOwnerSummary(referent: FreightReferent): string {
  ...
  const dock = req.reference ? ` for ${freightReferenceLabel(req.reference).toLowerCase()}` : ''
...
export function ambiguousFreightQuestion(options: FreightReferent[]): string {
  const labels = options.slice(0, 3).map(o => (o.workflow.request.reference ? freightReferenceLabel(o.workflow.request.reference) : null) || o.workflow.request.freightProvider || o.providerLabel).filter(Boolean)
