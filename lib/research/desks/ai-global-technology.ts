export const AI_GLOBAL_TECHNOLOGY_DESK = {
  ...
  cadence: { intervalHours: 6, reassessAfterMaterialChange: true },
  explorationBudget: {
  ...
  standingAreas: [
    ...
    'major startups', 'open-source AI', 'Chinese AI labs', 'Chinese technology ecosystem',
    'robotics', 'embodied AI', 'agents', 'computer use', 'multimodal systems', 'AI hardware',
    'inference', 'model economics', 'local models', 'agent protocols (MCP, A2A, and successors)',
    'autonomous software engineering', 'personal AI and JARVIS-style systems', 'important research papers',
  ],
  standingQuestions: [
  ...
  geographicScope: [
    'United States', 'China', 'Europe', 'United Kingdom', 'Canada', 'Japan', 'South Korea',
    'India', 'Southeast Asia', 'Middle East', 'Africa', 'Latin America', 'global open-source communities',
  ],
...
export type AiTechnologyEvidence = {
  ...
  /** Sources derived from the same upstream announcement share this key. */
...
export type AiTechnologyDevelopmentCandidate = {
  /** Stable semantic identity for the underlying development, never a URL. */
...
export type AiTechnologyDevelopment = {
  ...
  lastSeen: string
  evidence: AiTechnologyEvidence[]
  sourceQuality: { primaryCount: number; independentGroupCount: number; highQualityCount: number }
...
const MATERIALITY_KEYS = [
  'possibility', 'commoditization', 'builderDecision', 'opportunity', 'persistentAiTimeline',
] as const

const clamp = (value: number, min = 0, max = 1) => Math.max(min, Math.min(max, value))
const mean = (values: number[]) => values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : 0
const normalizedKey = (value: string) => value.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')
const sourceTimestamp = (item: AiTechnologyEvidence) => Date.parse(item.publishedAt ?? item.observedAt)

function validateCandidate(candidate: AiTechnologyDevelopmentCandidate): string {
  ...
  if (!candidate.title.trim() || !candidate.canonicalClaim.trim()) throw new Error(`AI technology development ${key} requires title and canonicalClaim`)
  if (!candidate.evidence.length) throw new Error(`AI technology development ${key} requires evidence`)
  if (candidate.evidence.some((item) => !item.id.trim() || !item.independenceKey.trim())) throw new Error(`AI technology development ${key} has invalid evidence identity`)
  if (candidate.evidence.some((item) => !Number.isFinite(sourceTimestamp(item)))) throw new Error(`AI technology development ${key} has invalid evidence timestamps`)
  if (MATERIALITY_KEYS.some((field) => !Number.isFinite(candidate.materiality[field]) || candidate.materiality[field] < 0 || candidate.materiality[field] > 1)) {
    throw new Error(`AI technology development ${key} materiality must be between 0 and 1`)
  }
  return key
}
...
function confidenceFor(evidence: AiTechnologyEvidence[]): number {
  ...
  if (!supports.length) return contradicts.length ? 0.08 : 0.15

  const supportGroups = new Map<string, number>()
  for (const item of supports) {
    supportGroups.set(item.independenceKey, Math.max(supportGroups.get(item.independenceKey) ?? 0, SOURCE_WEIGHT[item.sourceKind]))
  }
  const contradictionGroups = new Map<string, number>()
  for (const item of contradicts) {
    contradictionGroups.set(item.independenceKey, Math.max(contradictionGroups.get(item.independenceKey) ?? 0, SOURCE_WEIGHT[item.sourceKind]))
  }

  const strongest = Math.max(...supportGroups.values())
  const corroborationBoost = clamp((supportGroups.size - 1) * 0.08, 0, 0.2)
  const primaryBoost = supports.some((item) => item.sourceKind === 'primary') ? 0.08 : 0
  const contradictionPenalty = clamp([...contradictionGroups.values()].reduce((sum, value) => sum + value * 0.18, 0), 0, 0.45)
  const raw = strongest * 0.72 + corroborationBoost + primaryBoost - contradictionPenalty

  // One upstream source can be excellent evidence, but it is not independent
  // corroboration. Keep it below the desk's supported/high-confidence threshold.
  const corroborationCap = supportGroups.size >= 2 ? 0.98 : 0.69
  return Number(clamp(raw, 0.05, corroborationCap).toFixed(3))
}

function consolidateDevelopment(candidates: AiTechnologyDevelopmentCandidate[], previous?: PreviousAiTechnologyBelief): AiTechnologyDevelopment {
  const representative = candidates[0]
  const evidence = [...new Map(candidates.flatMap((candidate) => candidate.evidence).map((item) => [item.id, item])).values()]
    .sort((a, b) => SOURCE_WEIGHT[b.sourceKind] - SOURCE_WEIGHT[a.sourceKind] || sourceTimestamp(b) - sourceTimestamp(a))
  const timestamps = evidence.map(sourceTimestamp)
  const confidence = confidenceFor(evidence)
  const contradictions = evidence.filter((item) => item.stance === 'contradicts')
  const supportGroups = new Set(evidence.filter((item) => item.stance === 'supports').map((item) => item.independenceKey))
  const materiality = Object.fromEntries(MATERIALITY_KEYS.map((field) => [field, Math.max(...candidates.map((candidate) => candidate.materiality[field]))])) as AiTechnologyDevelopmentCandidate['materiality']
  const overall = Number(mean(MATERIALITY_KEYS.map((field) => materiality[field])).toFixed(3))
  const questionIndexes = [...new Set(candidates.flatMap((candidate) => candidate.standingQuestionIndexes))].sort((a, b) => a - b)
  const state: AiTechnologyDevelopment['belief']['state'] =
    contradictions.length > 0 && confidence < 0.66 ? 'contested'
      : previous && confidence < previous.confidence - 0.15 ? 'weakening'
  ...
  return {
    ...
    themes: [...new Set(candidates.flatMap((candidate) => candidate.themes.map(normalizedKey)).filter(Boolean))].sort(),
    standingQuestions: questionIndexes
    ...
    materiality: { ...materiality, overall },
  }
}
...
export function detectAiTechnologyTrends(developments: AiTechnologyDevelopment[]): AiTechnologyTrend[] {
  ...
  for (const development of developments) {
    for (const theme of development.themes) byTheme.set(theme, [...(byTheme.get(theme) ?? []), development])
  }
  ...
  return [...byTheme.entries()]
    ...
    .map(([theme, items]) => {
      ...
      return {
        ...
        regions,
        firstSeen: new Date(Math.min(...items.map((item) => Date.parse(item.firstSeen)))).toISOString(),
        lastSeen: new Date(Math.max(...items.map((item) => Date.parse(item.lastSeen)))).toISOString(),
        confidence: Number(clamp(mean(items.map((item) => item.belief.confidence)) + diversityBoost, 0, 0.98).toFixed(3)),
      }
    })
...
/**
 * Desk-specific projection over Caye's canonical research evidence. Persistence,
 * scheduling, runs, claims, and evidence remain owned by Research Runtime V1 / the
 * canonical intelligence substrate. This function returns stable JSON-safe state.
 */
export function compileAiGlobalTechnologyIntelligence(
  ...
  const previousByKey = new Map(previousBeliefs.map((belief) => [normalizedKey(belief.developmentKey), belief]))
  const groups = new Map<string, AiTechnologyDevelopmentCandidate[]>()

  for (const candidate of candidates) {
    const key = validateCandidate(candidate)
  ...
  return { deskId: AI_GLOBAL_TECHNOLOGY_DESK.id, generatedAt, developments, emergingTrends: detectAiTechnologyTrends(developments) }
}
