...
describe('research worker', () => {
  ...
  it('executes the canonical question and returns its id for lifecycle handling', async () => {
    ...
    expect(deps.executeRun).toHaveBeenCalledWith({
      runId: 'run-1', questionId: 'question-1', question: 'What is true?', provider, synthesize,
    })
    expect(result).toMatchObject({ status: 'completed', runId: 'run-1', questionId: 'question-1', sourceCount: 2, revision: 3 })
    expect(deps.failRun).not.toHaveBeenCalled()
  })

  it('retains question identity when a claimed run fails so lifecycle policy can retry safely', async () => {
    ...
    expect(result).toEqual({ status: 'failed', runId: 'run-1', questionId: 'question-1', error: 'Research question is unavailable' })
  })
