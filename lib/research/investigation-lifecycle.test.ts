...
describe('autonomous research investigation lifecycle', () => {
  ...
  it('unknown generates a bounded follow-up question', () => {
    const children = planInvestigationFollowUps({
      unknowns: ['Who owns the remaining stake?'],
  ...
  it('contradiction triggers an independent cross-check', () => {
    const children = planInvestigationFollowUps({
      unknowns: [],
      conflicting_evidence: [{ source: 'primary source disagrees' }],
      material_changes: [],
    })
    expect(children).toHaveLength(1)
  ...
  it('monitoring question gets a future wakeup', () => {
    const result = decideInvestigationLifecycle({
      question: question('monitor'),
      brief: { unknowns: [], conflicting_evidence: [], material_changes: ['new filing'] },
      now,
    })
  ...
  it('duplicate follow-up is suppressed with stable root-scoped identity', () => {
    const children = planInvestigationFollowUps({
      unknowns: ['Who owns the remaining stake?', ' Who owns the remaining stake? '],
      conflicting_evidence: [],
      material_changes: [],
    })
    expect(children).toHaveLength(1)
    expect(investigationFollowUpCanonicalKey('root-1', children[0]))
      .toBe(investigationFollowUpCanonicalKey('root-1', children[0]))
  })
