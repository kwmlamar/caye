 ...
 * the retry-on-contract-violation loop live here so that swapping OpenAI for
 * Anthropic cannot quietly change epistemic typing, confidence calibration, or
 * contradiction preservation.
 *
 * A provider contributes exactly one thing: turning (system, user) into text.
 */
...
/**
 * Build a synthesizer over any provider's completion call. Behavior is byte-for
 * -byte the same contract the Anthropic-only implementation enforced before the
