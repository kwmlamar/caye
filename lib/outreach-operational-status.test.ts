...
function state(patch: Partial<OutreachOperationalStatus> = {}): Omit<OutreachOperationalStatus, 'reasonNoOutreach'> {
  const base: OutreachOperationalStatus = {
    ...
    pause: { paused: false, source: 'unknown', reason: null, pausedAt: null, disposition: 'running' },
...
describe('authoritative outreach explanation', () => {
  ...
  it('reports a bounce safety pause as the concrete blocker', () => expect(explainNoOutreach(state({ paused: true, pause: { paused: true, source: 'bounce_kill_switch', reason: '5 bounces', pausedAt: null, disposition: 'safety_locked' } }))).toMatch(/bounce safety stop/))
