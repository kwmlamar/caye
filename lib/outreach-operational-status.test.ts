...
function state(patch: Partial<OutreachOperationalStatus> = {}): Omit<OutreachOperationalStatus, 'reasonNoOutreach'> {
  const base: OutreachOperationalStatus = {
    ...
    pause: { paused: false, source: 'unknown', reason: null, pausedAt: null, generation: null, activeSafetyCondition: null, disposition: 'running' },
...
describe('authoritative outreach explanation', () => {
  ...
  it('reports an active bounce safety pause as the concrete blocker', () => expect(explainNoOutreach(state({ paused: true, pause: { paused: true, source: 'bounce_safety', reason: '5 bounces', pausedAt: null, generation: 'pause-1', activeSafetyCondition: 'bounce_threshold', disposition: 'safety_active' } }))).toMatch(/bounce threshold safety stop/))
