...
export async function transcribeWhatsAppVoiceNote(
  ...
  if (!res.ok) throw new Error(`OpenAI voice-note transcription failed: ${res.status} ${(await res.text()).slice(0, 400)}`)
...
export async function synthesizeWhatsAppVoiceNote(text: string): Promise<Uint8Array> {
  const res = await fetch(`${OPENAI_BASE}/audio/speech`, {
    ...
    body: JSON.stringify({
      ...
      instructions: 'Speak naturally and conversationally as Caye. Keep the delivery concise, warm, and unhurried.',
    }),
  })
  if (!res.ok) throw new Error(`OpenAI voice-note synthesis failed: ${res.status} ${(await res.text()).slice(0, 400)}`)
...
export async function uploadWhatsAppAudio(
  ...
  if (!res.ok) throw new Error(`WhatsApp audio upload failed: ${res.status} ${(await res.text()).slice(0, 400)}`)
...
export async function sendWhatsAppVoiceNote(
  ...
  if (!res.ok) throw new Error(`WhatsApp voice-note send failed: ${res.status} ${(await res.text()).slice(0, 400)}`)
}
