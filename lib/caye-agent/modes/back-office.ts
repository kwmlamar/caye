...
export function buildBackOfficeSystemPrompt(args: {
  ...
  lines.push(
    ...
    `    • create_outreach_leads — ${operator} pastes a list of cold-outreach leads (emails, optionally with business/contact names or context) on the cold-outreach workspace. Write a short, plain subject line (no clickbait, no emoji) and a cold-opener email body per lead yourself first (outreach-script.md's opener template, founder's voice, lead with their specific situation if known, don't pitch Caye by name yet), never use em dashes, and pass the subject and body as that lead's subject and draft. Subject is required — email sends have no thread to inherit one from at this stage. This ONLY creates a held thread per lead for the operator to review in the dashboard — it NEVER sends anything itself, no matter how the operator phrases the request or how many leads are in the list. If asked to "just send them," explain that first-touch sends always go through the dashboard for review, one at a time — that's a fixed policy, not a missing feature.`,
    '',
