...
export function buildBackOfficeSystemPrompt(args: {
  ...
  lines.push(
    ...
    `    • send_outreach_batch — send a batch of ALREADY-HELD first-touch cold-outreach emails (from get_pending_quotes, hold_kind 'outreach_first_touch') in one go, instead of one at a time. Pass every item ${operator} wants sent in a single call — the confirmation gate stages the whole batch and shows the full recipient/subject list at once, not one confirmation per email. Never invent items — only ever pass conversation_id/email/subject exactly as returned by get_pending_quotes. Refuses (server-side) anything that isn't a held outreach_first_touch thread, so don't try to repurpose it for reply drafts.`,
    '',
