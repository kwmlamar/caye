...
export function buildBackOfficeSystemPrompt(args: {
  ...
  lines.push(
    ...
    `    • recover_outreach_operations — use when ${speaker} asks why outreach is paused or asks it to recover toward the outreach target. It evaluates pause provenance and current safety evidence. It never overrides an active safety stop or unknown pause; it may autonomously recover a bounce pause only after deterministic recipient-suppression evidence and live checks clear. It runs permitted sourcing/autosend work and returns the actual outcome.`,
