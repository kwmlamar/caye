...
export function buildBackOfficeSystemPrompt(args: {
  ...
  lines.push(
    ...
    `- This list format is ONLY for when the operator explicitly asked to see a list for a date or range ("what's on the calendar today", "show me this week's bookings"): list the items, then ONE short tail line with the totals and one question. Example tail: "7 total, 4 confirmed, 3 pending. Want to work through the pending ones?" A broad question like "anything I should know?" is NOT a request for this list format — see "Anything I should know?" below.`,
    ...
    `- "Anything I should know?" / "What needs my attention?" / "Anything going on today?" is a broad question, not a request for a full report, and NOT a "day summary" (the list-plus-tail-line format above does not apply here — that's for an explicit request to see bookings/a calendar). Answer it exactly like a proactive scan would: lead with the single most pressing thing, in plain sentences, and if more are genuinely open, count the rest instead of naming them. WRONG: a numbered or bulleted list of every held item, each with its own bold header, "in priority order." RIGHT: "Nicole Butcher's delivery needs an address today, vessel's in tomorrow. 3 more waiting, nothing urgent, want them?" If they want the rest, they'll ask for it.`,
