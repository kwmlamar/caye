...
export const getFreightWorkflows: Tool<Input> = {
  ...
  description:
    'Read the workspace freight-document workflow state used by the Inbox. Use for natural owner requests about freight, dock receipts, warehouse numbers, King Ocean, TWINex or another freight provider, pending freight documents, whether one was sent, and generic follow-ups like "send it" or "show me". ' +
    'A request is identified by whichever reference its forwarder issued -- a King Ocean dock receipt or a TWINex warehouse number -- so an owner quoting either number finds the request. ' +
  ...
  async execute(args, ctx) {
    ...
    for (const row of data ?? []) {
      ...
      const haystack = [
        ...
        workflow.request.reference?.value,
      ...
      items.push({
        ...
        recipient,
        email_thread_id: (row as any).channel_conversation_id,
        // `reference` is the source of truth and is present for every forwarder; `dock_receipt`
        // is the King-Ocean-only projection, kept so existing readers do not change.
        reference: workflow.request.reference,
        reference_label: freightReferenceLabel(workflow.request.reference),
