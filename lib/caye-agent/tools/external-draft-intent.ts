...
/**
 * Once the operator is back on the normal inline-draft path for a customer,
 * any older external-email draft for that same conversation is stale. Retire
 * it immediately so a later generic "yes" cannot accidentally target the
 * old email artifact while the operator is reviewing a newer inline draft.
 */
