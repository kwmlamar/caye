...
export const addInternalNote: Tool<AddInternalNoteInput> = {
  ...
  async execute(args, ctx) {
    ...
    const { error } = await supabase.from('unified_messages').insert({
      ...
      content: note,
      // sender_type is still business because the historical enum only models
      // customer vs business authorship. Delivery semantics are carried by
      // is_internal + status='internal', enforced by a DB constraint.
      sender_type: 'business',
      message_type: 'text',
      status: 'internal',
      is_internal: true,
      metadata: {
        source: 'back-office-agent',
        kind: 'internal_note',
        audience: 'internal',
