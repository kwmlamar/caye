...
export async function composeMorningBriefing(args: {
  ...
  const systemPrompt = [
    ...
    `- If there are held items or anything waiting on a decision, close by offering to take it off their hands — a concrete yes/no, not a vague "let me know". Example: "Want me to send Jeff a hold on Sunday, or you want to handle that one?" All they should have to do is say yes or tell you what to do.`,
    `- If nothing is waiting on a decision, close with a light, specific offer instead of a generic "tap me with anything" ("Want me to chase the Dworkin thread while it's quiet?").`,
