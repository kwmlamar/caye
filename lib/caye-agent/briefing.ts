...
export async function composeEodSummary(args: {
  ...
  const systemPrompt = [
    ...
    `- Held items with has_open_escalation=true already get their own daily "still waiting" ping from a separate system — don't name them or re-propose an action here. Fold them into a single short clause instead ("+ 2 still escalated from before, no change").`,
...
export async function composeMorningBriefing(args: {
  ...
  const systemPrompt = [
    ...
    `- Lead with anything that needs the operator's attention (held items, urgents) — but ONLY held items with has_open_escalation=false. Items with has_open_escalation=true already have their own daily "still waiting" ping from a separate system; naming them here too is duplicate noise the operator already saw. Fold those into one short clause instead ("+ 2 still escalated, no change") rather than describing each one again.`,
    `- Then today's calendar in plain language, not a bullet list.`,
    `- If there's a held item with has_open_escalation=false, close by offering to take it off their hands — a concrete yes/no, not a vague "let me know". Example: "Want me to send Jeff a hold on Sunday, or you want to handle that one?" All they should have to do is say yes or tell you what to do. Don't make this offer for already-escalated items — they already got that offer via the escalation ping.`,
    `- If nothing new is waiting on a decision, close with a light, specific offer instead of a generic "tap me with anything" ("Want me to chase the Dworkin thread while it's quiet?") — but only reference a specific thread this way if it's has_open_escalation=false; otherwise keep the offer generic.`,
    ...
    `- Don't ask a vague open-ended question ("let me know if you need anything") — every close should be a specific, answerable yes/no or short instruction tied to something real from the tools you called.`,
