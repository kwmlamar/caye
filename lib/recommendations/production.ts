...
function parseJsonObject(text: string): Record<string, unknown> {
  ...
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('recommendation proposer returned a non-object payload')
  }
...
export const createConfiguredRecommendationProposer = (): RecommendationProposer => async (context) => {
  const text = await routedCompletion({
    system: recommendationSystemPrompt(),
    user: JSON.stringify({
      trigger: context.trigger,
      goal: context.goal,
      intelligence: context.intelligence,
      canonicalGoalImpact: context.goalImpact,
      beliefRevisions: context.beliefRevisions,
      allowedIntelligenceIds: [context.intelligence.id],
      allowedClaimIds: context.goalImpact.evidenceClaimIds,
      allowedBeliefRevisionIds: context.beliefRevisions.map((revision) => revision.id),
    }),
...
function revisionSnapshot(revision: DbRevision): RecommendationBeliefRevisionSnapshot {
  return {
    id: revision.id,
    priorConfidence: revision.prior_confidence,
    revisedConfidence: revision.revised_confidence,
    evidenceRole: revision.evidence_role,
...
async function loadRecommendationCandidates(): Promise<RecommendationCandidate[]> {
  ...
  const impactsResult = await db
    .from('intelligence_goal_impacts')
    .select('intelligence_item_id,goal_id,mechanism,impact,confidence,evidence_claim_ids,synthesis_fingerprint,updated_at')
    .order('updated_at', { ascending: false })
    .limit(IMPACT_LOAD_LIMIT)
  ...
  const [goalsResult, itemsResult, revisionsResult] = await Promise.all([
    db.from('caye_goals')
      .select('id,title,description,status,superseded_at')
      .in('id', goalIds)
      .eq('status', 'active')
      .is('superseded_at', null),
    db.from('intelligence_items')
      .select('id,domain,canonical_claim,status,confidence,materiality,valid_until,provenance')
      .in('id', itemIds)
      .in('status', ['current', 'contested']),
    db.from('intelligence_belief_revisions')
      .select('id,intelligence_item_id,prior_confidence,revised_confidence,evidence_role,created_at')
      .in('intelligence_item_id', itemIds)
      .order('created_at', { ascending: false })
      .limit(REVISION_LOAD_LIMIT),
  ])
  ...
  return impacts.flatMap((impact) => {
    const goal = goals.get(impact.goal_id)
    const item = items.get(impact.intelligence_item_id)
    if (!goal || !item) return []
    return [{
      goal: {
        id: goal.id,
        ...
        status: goal.status,
        supersededAt: goal.superseded_at,
      },
      intelligence: {
        id: item.id,
        domain: item.domain,
      ...
      goalImpact: {
        ...
        evidenceClaimIds: impact.evidence_claim_ids ?? [],
        synthesisFingerprint: impact.synthesis_fingerprint,
      },
      beliefRevisions: revisions
        .filter((revision) => revision.intelligence_item_id === item.id)
        .map(revisionSnapshot),
  ...
  return {
    ...
    async hasProposalFingerprint(fingerprint) {
      ...
      const result = await db
        ...
        .select('id')
        .contains('provenance', { proposalFingerprint: fingerprint })
        .limit(1)
      if (result.error) throw result.error
      return Boolean(result.data?.length)
    },

    persist: createGroundedRecommendation,
  }
}

export async function runMaterialIntelligenceRecommendations() {
  return runMaterialRecommendationRuntime({
    store: createProductionRecommendationStore(),
    proposer: createConfiguredRecommendationProposer(),
  })
}
