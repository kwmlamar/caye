...
async function reconcileFinalization(
  ...
  if (jobError || artifactError) return { state: 'indeterminate', reason: `authoritative reconciliation query failed: ${jobError?.message ?? artifactError?.message ?? 'unknown error'}` }
  if (job?.status === 'completed' && artifact?.id === args.artifactId && artifact.job_id === args.jobId) {
    ...
    if (complete && Number.isInteger(artifact.revision)) return { state: 'committed', artifact: { artifactId: artifact.id, revision: artifact.revision, name: artifact.name ?? args.name } }
...
export async function createEngineeringArtifact(args: { workspaceId: string; threadId: string; messageId: string; spec: EngineeringSpec; parentArtifactId?: string | null; taskType: 'create_parametric_part' | 'revise_parametric_part' }) {
  ...
  const { data: job, error: jobError } = await supabase.from('engineering_jobs').insert({ workspace_id: args.workspaceId, originating_thread_id: args.threadId, originating_message_id: args.messageId, status: 'running', task_type: args.taskType, parameters: args.spec.parameters, assumptions: args.spec.assumptions, runtime: 'cadquery', provenance: { engine: 'cadquery', sandbox: 'vercel', schema_version: 1 }, started_at: new Date().toISOString() }).select('id').single()
  ...
  try {
    ...
    const inputs: Array<[FileKind, string, Buffer | string, string]> = [['source', 'part.py', generated.source, 'text/x-python'], ['preview_geometry', 'part.stl', generated.stl, 'application/sla'], ['export_geometry', 'part.step', generated.step, 'model/step'], ['metadata', 'metadata.json', JSON.stringify(generated.metadata), 'application/json']]
    ...
    const finalizeArgs = { p_job_id: job.id, p_workspace_id: args.workspaceId, p_artifact_id: artifactId, p_lineage_id: lineageId, p_parent_artifact_id: parent?.id ?? null, p_name: args.spec.name, p_parameters: args.spec.parameters, p_assumptions: args.spec.assumptions, p_dimensions: generated.metadata.bounds_mm, p_calculation_metadata: { volume_mm3: generated.metadata.volume_mm3, disclaimer: 'Geometry-derived properties only. This is not structural verification or a safe-load claim.' }, p_provenance: { engine: 'cadquery', generated_at: new Date().toISOString() }, p_files: files }
    ...
    if (!finalizeError && result?.artifact_id && Number.isInteger(result.revision)) return { artifactId: result.artifact_id, revision: result.revision, name: args.spec.name }
    ...
    if (reconciliation.state === 'indeterminate') throw new EngineeringFinalizationIndeterminateError(`Engineering finalization is indeterminate: ${reconciliation.reason}`, { cause: finalizeError ?? new Error('incomplete finalization result') })
    ...
    try { await markJobFailed(supabase, job.id, cause) } catch (transitionError) { throw new Error(`Engineering generation failed and failure state was not recorded: ${transitionError instanceof Error ? transitionError.message : String(transitionError)}`, { cause }) }
