...
/**
 * Writes operator-taught business knowledge into the existing business_facts
 * store, now enriched as typed operating memory. Conflict detection, semantic
 * dedup and service grounding remain the same proven mechanisms; the typed RPC
 * adds provenance, correction lineage, confidence and authority semantics.
 */
export async function writeBusinessFact(args: {
  ...
  const authorityKind = args.callerRole === 'founder'
    ? 'founder'
    : args.callerRole === 'owner'
      ? 'owner'
      : 'operator'

  const { data: rpcResult, error } = await supabase
    .rpc('write_typed_business_memory_atomic', {
