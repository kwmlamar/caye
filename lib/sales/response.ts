...
export function salesTools<T extends { name: string }>(tools: readonly T[]): T[] {
  return tools.filter((tool) => ['send_reply', 'hold_for_human', 'escalate_to_team'].includes(tool.name))
}
...
export function buildSalesResponseSystem(
  ...
  stable +=
    ...
    'Use hold_for_human when you have written something but are not confident it is right. Being unsure is a legitimate reason to stop; guessing is not.'
