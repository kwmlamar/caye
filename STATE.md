...
- **Draft-before-send, always.** Any customer-facing send (email/WhatsApp/IG/Messenger) must stage a draft and wait for a separate, explicit operator confirmation before it goes out — for every workspace, not just the ones that ask for "a draft." A direct instruction ("tell him $75") must produce a draft first, exactly like an explicit draft request would. Enforced in code, not prompt text, at every send-capable path (`lib/whatsapp/actions/edit.ts` / `actionSend`; the back-office agent's `send_reply` via `lib/caye-agent/tools/high-risk-gate.ts`). (decisions-log 2026-08-06)
