-- Run the existing canonical workspace event perception observer across bounded active workspaces.
-- This is a scheduler bridge, not a second event bus.
...
begin
  for v_source in
    select distinct on (workspace_id) workspace_id, id, occurred_at, type
...
revoke all on function public.run_workspace_event_perception_cycle(integer) from public;
...
'Runs the canonical workspace-event perception observer for a bounded set of real workspace sources. Failures are isolated per workspace and retried by the scheduler.';
