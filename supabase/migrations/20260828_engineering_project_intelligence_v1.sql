...
begin
  select property_id into project_property_id from public.engineering_projects p
  ...
  if tg_table_name = 'engineering_project_predictions' and not exists (
    select 1 from public.engineering_project_alternatives a
    where a.id = new.alternative_id and a.project_id = new.project_id and a.workspace_id = new.workspace_id
  ) then raise exception 'prediction alternative is not part of this project'; end if;

  if tg_table_name = 'engineering_project_decisions' then
    if not exists (select 1 from public.engineering_project_alternatives a where a.id = new.alternative_id and a.project_id = new.project_id and a.workspace_id = new.workspace_id)
      then raise exception 'decision alternative is not part of this project'; end if;
    if not exists (select 1 from public.caye_operator_messages m where m.id = new.source_message_id and m.workspace_id = new.workspace_id and m.direction = 'inbound' and m.origin = 'dashboard')
      then raise exception 'engineering decision source must be an inbound founder dashboard message in this workspace'; end if;
  end if;

  if tg_table_name = 'engineering_project_execution_evidence' then
    if new.alternative_id is not null and not exists (select 1 from public.engineering_project_alternatives a where a.id = new.alternative_id and a.project_id = new.project_id and a.workspace_id = new.workspace_id)
      then raise exception 'execution alternative is not part of this project'; end if;
    if not exists (select 1 from public.caye_operator_messages m where m.id = new.source_message_id and m.workspace_id = new.workspace_id and m.direction = 'inbound' and m.origin = 'dashboard')
      then raise exception 'execution source must be an inbound founder dashboard message in this workspace'; end if;
    if new.source_artifact_id is not null and not exists (select 1 from public.business_artifacts a where a.id = new.source_artifact_id and a.workspace_id = new.workspace_id)
      then raise exception 'execution artifact is not in this workspace'; end if;
    if new.installed_asset_id is not null and not exists (select 1 from public.property_assets a where a.id = new.installed_asset_id and a.workspace_id = new.workspace_id and a.property_id = project_property_id)
      then raise exception 'installed asset is not part of this project property'; end if;
  end if;

  if tg_table_name = 'engineering_project_outcomes' and not exists (
    select 1 from public.property_observations o
    where o.id = new.property_observation_id and o.workspace_id = new.workspace_id and o.property_id = project_property_id
  ) then raise exception 'outcome observation is not part of this project property'; end if;

  if tg_table_name = 'engineering_project_verdicts' and not exists (
    select 1 from public.caye_operator_messages m where m.id = new.source_message_id and m.workspace_id = new.workspace_id and m.direction = 'inbound' and m.origin = 'dashboard'
  ) then raise exception 'verdict source must be an inbound founder dashboard message in this workspace'; end if;
...
begin
  select b.status, b.workspace_id, p.property_id into baseline_status, baseline_workspace, project_property_id
  from public.engineering_project_baselines b join public.engineering_projects p on p.id = b.project_id
  where b.id = coalesce(new.baseline_id, old.baseline_id);
  ...
  if tg_op <> 'DELETE' and not exists (
    select 1 from public.property_observations o where o.id = new.property_observation_id and o.workspace_id = new.workspace_id and o.property_id = project_property_id
  ) then raise exception 'baseline observation is not part of this project property'; end if;
...
create trigger engineering_baseline_items_scope_and_immutable before insert or update or delete on public.engineering_project_baseline_items
for each row execute function public.caye_assert_engineering_baseline_item_scope();
