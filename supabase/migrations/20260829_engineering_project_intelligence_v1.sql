...
create table if not exists public.engineering_project_baselines (
  ...
  revision integer not null default 1 check (revision > 0), status text not null default 'draft' check (status in ('draft','frozen')),
  notes text, frozen_at timestamptz, created_at timestamptz not null default now(),
  unique (workspace_id, project_id, revision), unique (workspace_id, id),
...
create table if not exists public.engineering_project_baseline_items (
  id uuid primary key default gen_random_uuid(), workspace_id uuid not null, baseline_id uuid not null,
  property_observation_id uuid not null references public.property_observations(id), created_at timestamptz not null default now(),
  unique (baseline_id, property_observation_id), foreign key (workspace_id, baseline_id) references public.engineering_project_baselines(workspace_id, id) on delete cascade
);
create table if not exists public.engineering_project_alternatives (
  id uuid primary key default gen_random_uuid(), workspace_id uuid not null, project_id uuid not null,
  alternative_key text not null check (length(trim(alternative_key)) between 1 and 120), revision integer not null default 1 check (revision > 0),
  title text not null check (length(trim(title)) between 1 and 240), description text not null check (length(trim(description)) between 1 and 8000),
  assumptions jsonb not null default '[]'::jsonb check (jsonb_typeof(assumptions) = 'array'), dependencies jsonb not null default '[]'::jsonb check (jsonb_typeof(dependencies) = 'array'),
  status text not null default 'candidate' check (status in ('candidate','selected','rejected','superseded')), estimated_cost numeric, cost_currency text, created_at timestamptz not null default now(),
  unique (workspace_id, project_id, alternative_key, revision), unique (workspace_id, id),
  foreign key (workspace_id, project_id) references public.engineering_projects(workspace_id, id) on delete cascade,
...
create table if not exists public.engineering_project_predictions (
  id uuid primary key default gen_random_uuid(), workspace_id uuid not null, project_id uuid not null, alternative_id uuid not null,
  metric_key text not null check (length(trim(metric_key)) between 1 and 160), numeric_value numeric not null, unit text not null check (length(trim(unit)) between 1 and 64),
  provenance_status text not null check (provenance_status in ('operator_confirmed','inferred','estimated')), confidence numeric check (confidence is null or (confidence >= 0 and confidence <= 1)),
  rationale text, analysis_ref text, artifact_ref text, created_at timestamptz not null default now(), unique (alternative_id, metric_key),
  foreign key (workspace_id, project_id) references public.engineering_projects(workspace_id, id) on delete cascade,
...
create table if not exists public.engineering_project_decisions (
  id uuid primary key default gen_random_uuid(), workspace_id uuid not null, project_id uuid not null, alternative_id uuid not null,
  source_message_id uuid not null references public.caye_operator_messages(id), rationale text, selected_at timestamptz not null default now(), superseded_at timestamptz,
  foreign key (workspace_id, project_id) references public.engineering_projects(workspace_id, id) on delete cascade,
...
create table if not exists public.engineering_project_execution_evidence (
  id uuid primary key default gen_random_uuid(), workspace_id uuid not null, project_id uuid not null, alternative_id uuid,
  evidence_type text not null check (evidence_type in ('operator_confirmation','artifact','installed_asset')), source_message_id uuid not null references public.caye_operator_messages(id),
  source_artifact_id uuid references public.business_artifacts(id), installed_asset_id uuid references public.property_assets(id), notes text, occurred_at timestamptz not null, created_at timestamptz not null default now(),
  foreign key (workspace_id, project_id) references public.engineering_projects(workspace_id, id) on delete cascade,
  foreign key (workspace_id, alternative_id) references public.engineering_project_alternatives(workspace_id, id),
...
create table if not exists public.engineering_project_outcomes (
  id uuid primary key default gen_random_uuid(), workspace_id uuid not null, project_id uuid not null,
  metric_key text not null check (length(trim(metric_key)) between 1 and 160), property_observation_id uuid not null references public.property_observations(id), created_at timestamptz not null default now(),
  unique (project_id, metric_key, property_observation_id), foreign key (workspace_id, project_id) references public.engineering_projects(workspace_id, id) on delete cascade
);
create table if not exists public.engineering_project_verdicts (
  id uuid primary key default gen_random_uuid(), workspace_id uuid not null, project_id uuid not null,
  verdict text not null check (verdict in ('succeeded','partially_succeeded','failed','inconclusive')), reason_codes jsonb not null default '[]'::jsonb check (jsonb_typeof(reason_codes) = 'array'),
  summary text not null check (length(trim(summary)) between 1 and 8000), source_message_id uuid not null references public.caye_operator_messages(id), created_at timestamptz not null default now(), superseded_at timestamptz,
  foreign key (workspace_id, project_id) references public.engineering_projects(workspace_id, id) on delete cascade
);
...
returns trigger language plpgsql set search_path = public as $$ begin if old.status = 'frozen' then raise exception 'Frozen engineering baseline is immutable'; end if; if tg_op = 'DELETE' then return old; else return new; end if; end $$;
