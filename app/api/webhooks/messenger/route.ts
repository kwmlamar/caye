...
async function processInboundMessenger(payload: Record<string, unknown>): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    for (const event of messagingEvents) {
      ...
      // Upsert a contact row for this Messenger sender, keyed on
      // (workspace, channel, PSID) — mirrors the email webhook's contact
      // upsert (zoho-email/route.ts) so Caye's per-customer style learning
      // also works for Messenger.
      ...
      if (contactErr) {
        console.warn('[messenger webhook] Contact upsert failed (continuing):', contactErr.message)
      }
      ...
      if (!existing) {
        ...
        if (inboundErr) {
          ...
          maybeRefreshContactProfile(conversation.contact_id).catch(err =>
            console.error('[messenger webhook] Contact profile refresh failed:', err)
          )
        }
      }

      if (aiConfig?.ai_enabled === false) {
        console.log(`[messenger webhook] AI disabled for workspace ${workspaceId} — skipping auto-reply`)
        continue
      }
      ...
      if (decision.bookingId) {
        ...
        }).catch(err => console.error('[messenger webhook] enqueueBookingCreated failed:', err))
        console.log(
