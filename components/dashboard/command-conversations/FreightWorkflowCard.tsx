...
import type { FreightReference } from '@/lib/freight/types'
...
type State = { isFreightDocumentRequest?: boolean; status?: 'MATCH_FOUND' | 'AMBIGUOUS' | 'NO_MATCH' | 'READY_FOR_APPROVAL' | 'SENT'; request?: { reference: FreightReference | null; freightProvider: string | null }; candidates?: Candidate[]; selectedEvidenceId?: string | null; generatedArtifactId?: string | null; reply?: string | null; error?: string }
...
export function FreightWorkflowCard({ workspaceId, conversationId, onPrepared }: { workspaceId: string; conversationId: string; onPrepared: (reply: string) => void }) {
  ...
  return <div style={{ margin: '0 18px 10px', padding: '12px 14px', border: '1px solid rgba(78,190,206,.25)', borderRadius: 14, background: 'rgba(78,190,206,.06)', color: TEXT }}>
    ...
    <div style={{ marginTop: 5, fontSize: 13, fontWeight: 700 }}>{state.request?.freightProvider ?? 'Freight provider'} needs a document for {freightReferenceLabel(state.request?.reference ?? null)}.</div>
