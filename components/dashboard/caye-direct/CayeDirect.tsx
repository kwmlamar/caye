        ...
        <div className="caye-overlay-main">
          ...
          <div className="caye-overlay-conversation">
            <div className={`caye-overlay-pane ${mode !== 'chat' ? 'is-hidden' : ''}`} aria-hidden={mode !== 'chat'}>
              {selected ? <CayeDirectThread key={selected.id} mode="thread" workspaceId={workspaceId} threadId={selected.id} threadTitle={selected.title} compactHeader autoFocusComposer={open && mode === 'chat'} composerVisible={mode === 'chat'} onThreadMeta={(meta) => updateThreadMeta(selected.id, meta)} onArchive={() => archiveThread(selected.id)} initialMessage={pendingInitialThreadId === selected.id ? pendingMessage : null} onInitialMessageSent={() => { setPendingInitialThreadId(null); setPendingMessage(null); onInitialMessageSent?.() }} /> : <div className="caye-overlay-empty">{threads === null ? <CayeLoadingPulse size={18} /> : <div className="caye-overlay-empty-inner"><CayeMark size={34} /><h2>How can I help?</h2><p>Ask about the business, a decision, or anything Caye is working on.</p><form className="caye-overlay-empty-form" onSubmit={(event) => { event.preventDefault(); startFromEmptyState() }}><input ref={emptyInputRef} value={emptyDraft} onChange={(event) => setEmptyDraft(event.target.value)} placeholder="Ask Caye anything…" aria-label="Ask Caye anything" /><button className="caye-overlay-empty-send" type="submit" disabled={!emptyDraft.trim() || creating} aria-label="Send"><svg aria-hidden width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round"><path d="M12 19V5M5 12l7-7 7 7" /></svg></button></form></div>}</div>}
            </div>
            <div className={`caye-overlay-pane ${mode !== 'live' ? 'is-hidden' : ''}`} aria-hidden={mode !== 'live'}>
              {selectedOperator ? <CayeDirectThread key={selectedOperator.id} mode="operator" workspaceId={workspaceId} operatorId={selectedOperator.id} operatorLabel={selectedOperator.name || 'Operator'} /> : <div className="caye-overlay-empty">{operators === null ? <CayeLoadingPulse size={18} /> : <div className="caye-overlay-empty-inner"><CayeMark size={34} /><h2>Live conversations</h2><p>Select a person to view their read-only WhatsApp conversation with Caye.</p></div>}</div>}
            </div>
          </div>
        </div>
      </section>
