...
function Block({ block, resolved }: { block: RichResultBlock; resolved?: Record<string, string> }) {
  ...
  return <a href={block.url} target="_blank" rel="noreferrer" style={{ color: '#72cfd9', fontSize: 12 }}>{block.name}</a>
}
export function RichResultRenderer({ result }: { result: RichResult }) { return <div style={{ display: 'grid', gap: 8, marginTop: 8 }}>{result.blocks.map((b, i) => <Block key={i} block={b} />)}</div> }
