...
function valueFor(o: Observation) {
  return typeof o.numeric_value === 'number' ? `${o.numeric_value}${o.unit ? ` ${o.unit}` : ''}` : o.text_value ?? '—'
}
...
function Compact({ snapshot, onExpand }: { snapshot: Snapshot; onExpand: () => void }) {
  ...
  return <div style={{ ...panel, padding: 14, display: 'grid', gap: 12 }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
      ...
      <ExpandResultButton onClick={onExpand} label={`Open ${snapshot.property.name} spatial twin`} />
    </div>
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 7 }}>
      {[
        ...
        ['Sensors', sensors ? `${sensors} planned` : 0],
      ].map(([label, value]) => <div key={String(label)} style={{ padding: '8px 9px', borderRadius: 9, background: 'rgba(255,255,255,.035)' }}><div style={{ fontSize: 9, color: '#71717a' }}>{label}</div><div style={{ fontSize: 13, fontWeight: 650, marginTop: 2 }}>{value}</div></div>)}
    </div>
  </div>
}
...
function HousePlan({ snapshot, onSelectAsset, selectedAssetId }: { snapshot: Snapshot; onSelectAsset: (id: string) => void; selectedAssetId?: string }) {
  ...
  return <div style={{ borderRadius: 14, overflow: 'hidden', border: '1px solid rgba(255,255,255,.08)', background: '#0e1214' }}>
    <svg viewBox="0 0 660 500" style={{ display: 'block', width: '100%', minHeight: 390 }} role="img" aria-label="Schematic Main House floor plan">
      {ROOMS.map(room => <g key={room.name}>
        <rect x={room.x} y={room.y} width={room.w} height={room.h} rx="7" fill="#151b1d" stroke="#3b4649" strokeWidth="2" />
        <text x={room.x + 10} y={room.y + 20} fill="#a9afb2" fontSize="12" fontWeight="600">{room.name}</text>
        {assets.filter(a => roomName(a) === room.name || (room.name === 'Living / Kitchen' && ['Kitchen', 'Living / Kitchen'].includes(roomName(a) ?? ''))).map((asset, index) => {
          const col = index % 3
          const row = Math.floor(index / 3)
          const x = room.x + 16 + col * 58
          const y = room.y + 48 + row * 42
          return <g key={asset.id} onClick={() => onSelectAsset(asset.id)} style={{ cursor: 'pointer' }}><circle cx={x} cy={y} r="8" fill={selectedAssetId === asset.id ? aqua : statusColor(asset.status)} /><text x={x + 13} y={y + 4} fill="#c7cbcd" fontSize="8.8">{asset.name.replace(room.name, '').trim() || asset.asset_type}</text></g>
        })}
      </g>)}
...
function SatellitePanel({ snapshot }: { snapshot: Snapshot }) {
  ...
  const hasAnchor = typeof geo?.lat === 'number' && typeof geo?.lng === 'number'
  const [src, setSrc] = useState<string | null>(null)
  const [state, setState] = useState<'idle' | 'loading' | 'ready' | 'error'>(hasAnchor ? 'loading' : 'idle')
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const res = await fetch(`/api/founder/property-snapshots/${encodeURIComponent(snapshot.property.id)}/satellite`, {
          headers: { Authorization: `Bearer ${session.access_token}` },
          cache: 'no-store',
        })
        if (!res.ok) throw new Error('Satellite image unavailable')
        const blob = await res.blob()
        if (!blob.type.startsWith('image/')) throw new Error('Invalid satellite image')
  ...
  }, [hasAnchor, snapshot.property.id])

  return <div style={{ ...panel, minHeight: 430, overflow: 'hidden', position: 'relative', display: 'grid', placeItems: 'center', background: '#0d1112' }}>
    {state === 'ready' && src && <img src={src} alt={`Satellite view centered on ${snapshot.property.name}`} style={{ width: '100%', height: '100%', minHeight: 430, objectFit: 'cover' }} />}
    {state === 'loading' && <div style={{ color: '#858790', fontSize: 11 }}>Loading satellite imagery…</div>}
    {!hasAnchor && <div style={{ maxWidth: 520, padding: 32, textAlign: 'center' }}><div style={{ fontSize: 14, fontWeight: 650 }}>Satellite anchor not configured yet</div><div style={{ color: '#858790', fontSize: 11, lineHeight: 1.6, marginTop: 8 }}>This property needs a founder-confirmed latitude/longitude before Caye can anchor external imagery.</div></div>}
    {hasAnchor && state === 'error' && <div style={{ maxWidth: 520, padding: 32, textAlign: 'center' }}><div style={{ fontSize: 14, fontWeight: 650 }}>Satellite imagery unavailable</div><div style={{ color: '#858790', fontSize: 11, lineHeight: 1.6, marginTop: 8 }}>The property is georeferenced, but the server could not retrieve imagery. The Google Maps key remains server-side and is never exposed to the dashboard.</div></div>}
    {hasAnchor && <div style={{ position: 'absolute', right: 12, top: 12, padding: '6px 8px', borderRadius: 8, background: 'rgba(5,8,9,.78)', color: '#b0b4b6', fontSize: 9.5 }}>{geo!.lat!.toFixed(6)}, {geo!.lng!.toFixed(6)}</div>}
...
function SpatialTwin({ snapshot }: { snapshot: Snapshot }) {
  ...
  return <div style={{ minHeight: '100%', display: 'grid', gridTemplateRows: 'auto minmax(0,1fr)', gap: 12, color: '#f4f4f5' }}>
    <header style={{ ...panel, padding: '12px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 14 }}>
      <div><div style={{ fontSize: 10, color: '#7f8189', textTransform: 'uppercase', letterSpacing: '.08em' }}>Property Intelligence · spatial twin v0.2</div><div style={{ fontSize: 20, fontWeight: 680 }}>{snapshot.property.name}</div></div>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>{(['site', 'house', '3d', 'satellite'] as ViewMode[]).map(mode => <button key={mode} type="button" onClick={() => setView(mode)} style={{ padding: '6px 9px', borderRadius: 8, border: view === mode ? '1px solid rgba(125,216,224,.55)' : '1px solid rgba(255,255,255,.08)', background: view === mode ? 'rgba(125,216,224,.08)' : 'transparent', color: view === mode ? aqua : '#9b9da4', cursor: 'pointer', fontSize: 10 }}>{mode === '3d' ? '3D' : titleCase(mode)}</button>)}<StatusPill status={snapshot.property.status} /></div>
    </header>
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 320px', gap: 12, minHeight: 0 }}>
      <main style={{ minWidth: 0, overflow: 'auto', display: 'grid', alignContent: 'start', gap: 12 }}>
        ...
        {water && <section style={{ ...panel, padding: 12 }}>
          <div style={{ fontSize: 10, color: muted, textTransform: 'uppercase', letterSpacing: '.06em' }}>Water overlay</div>
          <div style={{ display: 'flex', gap: 6, overflowX: 'auto', marginTop: 8 }}>{waterAssets.map((asset, index) => <div key={asset.id} style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>{index > 0 && <span style={{ color: '#4f8d93' }}>→</span>}<button type="button" onClick={() => setSelection({ kind: 'asset', id: asset.id })} style={{ minWidth: 112, padding: '8px 9px', borderRadius: 9, border: '1px solid rgba(255,255,255,.07)', background: selectedAsset?.id === asset.id ? 'rgba(125,216,224,.08)' : 'rgba(255,255,255,.025)', color: '#d8d9dd', cursor: 'pointer', textAlign: 'left', fontSize: 10.5 }}>{asset.name}<div style={{ color: statusColor(asset.status), fontSize: 9, marginTop: 2 }}>{asset.status.replaceAll('_', ' ')}</div></button></div>)}</div>
          <div style={{ color: '#777983', fontSize: 10, marginTop: 8 }}>{typeof water.metadata?.layout === 'string' ? water.metadata.layout : 'roof catchment → tanks → pump → pressure vessel → filtration → house'}</div>
        </section>}
        <section style={{ ...panel, padding: 12 }}>
          <div style={{ fontSize: 10, color: muted, textTransform: 'uppercase', letterSpacing: '.06em' }}>Live reality</div>
          <div style={{ fontSize: 13, fontWeight: 620, marginTop: 2 }}>Sensors & telemetry</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(190px,1fr))', gap: 8, marginTop: 9 }}>{sensors.map(sensor => {
            const asset = snapshot.assets.find(a => a.id === sensor.asset_id)
            const values = telemetry.filter(t => t.device_id === sensor.id)
            return <div key={sensor.id} style={{ padding: 10, borderRadius: 10, border: '1px solid rgba(255,255,255,.07)' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}><strong style={{ fontSize: 11 }}>{asset?.name ?? sensor.device_key}</strong><StatusPill status={sensor.status} /></div>{values.length ? values.map(v => <div key={v.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, marginTop: 7 }}><span style={{ color: muted }}>{titleCase(v.metric_key)}</span><span>{v.numeric_value} {v.unit}</span></div>) : <div style={{ color: '#6d7078', fontSize: 10, marginTop: 8 }}>No live reading yet.</div>}</div>
          })}</div>
        </section>
      </main>
      <aside style={{ ...panel, padding: 13, overflow: 'auto' }}>
        <div style={{ color: muted, fontSize: 9.5, textTransform: 'uppercase', letterSpacing: '.07em' }}>Inspector</div>
        <div style={{ fontSize: 17, fontWeight: 650, marginTop: 3 }}>{entityName ?? snapshot.property.name}</div>
        <div style={{ marginTop: 7 }}><StatusPill status={entityStatus} /></div>
        {selectedAsset && <div style={{ display: 'grid', gap: 4, marginTop: 14, fontSize: 10.5 }}><Info label="Type" value={selectedAsset.asset_type} />{selectedStructure && <Info label="Structure" value={selectedStructure.name} />}{selectedSystem && <Info label="System" value={selectedSystem.name} />}{selectedAsset.manufacturer && <Info label="Manufacturer" value={selectedAsset.manufacturer} />}</div>}
        <div style={{ marginTop: 16, color: muted, fontSize: 9.5, textTransform: 'uppercase' }}>Current known state</div>
        {observations.length ? observations.slice(0, 16).map(o => <div key={o.id} style={{ padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,.055)' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 7, fontSize: 9.5 }}><span style={{ color: '#92949c' }}>{titleCase(o.observation_key)}</span><span style={{ color: o.provenance_status === 'operator_confirmed' ? aqua : '#b7a65e' }}>{o.provenance_status}</span></div><div style={{ fontSize: 11.5, lineHeight: 1.45, marginTop: 3 }}>{valueFor(o)}</div>{o.notes && <div style={{ color: '#6d7078', fontSize: 9.5, lineHeight: 1.4, marginTop: 3 }}>{o.notes}</div>}</div>) : <div style={{ color: '#676a72', fontSize: 10.5, marginTop: 8 }}>Select a structure or asset to inspect its recorded state.</div>}
      </aside>
    </div>
  </div>
}
...
function Info({ label, value }: { label: string; value: string }) {
  return <div style={{ display: 'grid', gridTemplateColumns: '85px 1fr', gap: 8 }}><span style={{ color: muted }}>{label}</span><span>{value}</span></div>
}
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const { session } = await getSession()
        if (!session) throw new Error('Founder session unavailable')
        const res = await fetch(`/api/founder/property-snapshots/${encodeURIComponent(propertyId)}?workspaceId=${encodeURIComponent(workspaceId)}`, { headers: { Authorization: `Bearer ${session.access_token}` } })
