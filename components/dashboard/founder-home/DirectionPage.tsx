...
function useDirectionData(workspaceId: string) {
  ...
  const load = useCallback(async () => {
    ...
    setData({ operatorGoals: goalsJson.operatorGoals ?? [], workspaceGoals: goalsJson.workspaceGoals ?? [], capabilities: capabilitiesJson.capabilities ?? [] })
    setLoading(false)
  }, [workspaceId])
...
function CapabilityCard({ capability }: { capability: Capability }) {
  ...
  const color = MATURITY_COLOR[capability.maturityStatus]
  const verifiedEvidence = capability.evidence.filter((item) => item.verifies_capability)

  return <div style={{ ...glass(0.035), borderRadius: 12, padding: '12px 14px' }}>
    <button type="button" onClick={() => setExpanded((value) => !value)} style={{ width: '100%', border: 0, background: 'transparent', padding: 0, textAlign: 'left', cursor: 'pointer', color: TEXT }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
        <span aria-hidden style={{ width: 7, height: 7, borderRadius: '50%', flexShrink: 0, background: color, boxShadow: capability.maturityStatus === 'active' ? `0 0 7px ${color}88` : undefined }} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}><span style={{ fontSize: 13, fontWeight: 650 }}>{capability.title}</span><span style={{ fontSize: 10.5, color, fontWeight: 700 }}>{capability.maturityStatus.toUpperCase()}</span>{capability.progressPercent !== null && <span style={{ fontSize: 10.5, color: AQUA }}>{capability.progressPercent}% verified progress</span>}</div>
    ...
    {expanded && <div style={{ marginTop: 11, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.055)', display: 'grid', gap: 9 }}>
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', fontSize: 10.5, color: TEXT_QUIET }}><span>{verifiedEvidence.length} verified evidence item{verifiedEvidence.length === 1 ? '' : 's'}</span>{capability.lastVerifiedAt ? <span>last verified {new Date(capability.lastVerifiedAt).toLocaleDateString()}</span> : <span>not yet verified</span>}</div>
      {capability.limitations.length > 0 && <div style={{ fontSize: 11, color: ROSE }}>Needs work: {capability.limitations.join('; ')}</div>}
      {capability.relatedObjectives.length > 0 && <div style={{ fontSize: 11, color: TEXT_MUTED }}><span style={{ color: TEXT_QUIET }}>Goals · </span>{capability.relatedObjectives.map((link) => link.goal.title).join(' · ')}</div>}
      {capability.relatedInitiatives.length > 0 && <div style={{ fontSize: 11, color: TEXT_MUTED }}><span style={{ color: AQUA }}>Work in progress · </span>{capability.relatedInitiatives.map((link) => link.goal.title).join(' · ')}</div>}
      {capability.dependencies.length > 0 && <div style={{ fontSize: 11, color: TEXT_MUTED }}><span style={{ color: TEXT_QUIET }}>Needs · </span>{capability.dependencies.map((dependency) => dependency.capability.title).join(' · ')}</div>}
      {capability.evidence.length === 0 ? <div style={{ fontSize: 10.5, color: TEXT_QUIET, fontStyle: 'italic' }}>Not verified yet.</div> : capability.evidence.map((item) => <div key={item.id} style={{ fontSize: 10.5, color: TEXT_MUTED, padding: '3px 0', display: 'grid', gridTemplateColumns: '76px 1fr', gap: 8 }}><span style={{ color: item.verifies_capability ? EMERALD : TEXT_QUIET }}>{item.verifies_capability ? 'verified' : 'supporting'}</span><span><span style={{ color: TEXT }}>{item.summary}</span> · {item.evidence_kind} · {item.source_ref}</span></div>)}
    </div>}
  </div>
}
...
function CapabilitiesSection({ capabilities }: { capabilities: Capability[] }) {
  ...
  return <section style={{ marginBottom: 32 }}>
    <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}><div><div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '0.055em', color: TEXT_QUIET }}>CAPABILITY FOUNDATION</div><div style={{ fontSize: 11.5, color: TEXT_MUTED, marginTop: 4 }}>Secondary view: what the architecture can currently support, with evidence and limitations.</div></div><div style={{ fontSize: 10.5, color: TEXT_QUIET, whiteSpace: 'nowrap' }}>{verified}/{capabilities.length} verified</div></div>
    <div style={{ display: 'grid', gap: 8 }}>{capabilities.map((capability) => <CapabilityCard key={capability.id} capability={capability} />)}</div>
  </section>
}
...
function GoalNode({ goal, depth, byParent }: { goal: Goal; depth: number; byParent: Map<string, Goal[]> }) {
  ...
  return <div style={{ marginLeft: depth * 16 }}>
    <button type="button" onClick={() => children.length > 0 && setExpanded((value) => !value)} style={{ width: '100%', display: 'flex', alignItems: 'flex-start', gap: 8, textAlign: 'left', padding: '7px 8px', border: 0, borderRadius: 8, cursor: children.length > 0 ? 'pointer' : 'default', background: 'transparent', color: TEXT }}>
      ...
      <span style={{ flex: 1, minWidth: 0 }}><span style={{ fontSize: 13, fontWeight: goal.kind === 'vision' || goal.kind === 'domain' ? 600 : 500 }}>{goal.kind === 'domain' ? goal.title.toUpperCase() : goal.title}</span><span style={{ marginLeft: 8, fontSize: 10.5, color: TEXT_QUIET }}>{STATUS_LABEL[goal.status]}{goal.priority !== 'medium' ? ` · ${goal.priority}` : ''}</span>{goal.description && <div style={{ fontSize: 11.5, color: TEXT_MUTED, marginTop: 2 }}>{goal.description}</div>}{progress && <div style={{ fontSize: 11, color: TEXT_QUIET, marginTop: 2 }}>{progress}</div>}{!progress && goal.completionCriteria && <div style={{ fontSize: 11, color: TEXT_QUIET, marginTop: 2, fontStyle: 'italic' }}>Done when: {goal.completionCriteria}</div>}</span>
    </button>
...
function TreeSection({ title, goals }: { title: string; goals: Goal[] }) {
  const byParent = useMemo(() => {
    ...
    for (const goal of goals) { if (!goal.parentId) continue; const list = map.get(goal.parentId) ?? []; list.push(goal); map.set(goal.parentId, list) }
...
export default function DirectionPage({ workspaceId }: { workspaceId: string }) {
  ...
  async function handleSeed() {
    setSeeding(true)
    try { const { session } = await getSession(); if (!session) return; await fetch('/api/founder/goals/seed', { method: 'POST', headers: { Authorization: `Bearer ${session.access_token}` } }); await refetch() }
  ...
  const activeFocus = useMemo(() => {
    ...
    return all.filter((goal) => goal.status === 'active' && (goal.kind === 'objective' || goal.kind === 'goal' || goal.kind === 'initiative')).sort((a, b) => PRIORITY_WEIGHT[b.priority] - PRIORITY_WEIGHT[a.priority]).slice(0, 8)
  }, [data])
