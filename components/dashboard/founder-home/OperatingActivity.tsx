...
export default function OperatingActivity({ workspaceId }: { workspaceId: string }) {
  ...
  const state = STATUS[data?.status ?? 'idle']; const counts = useMemo(() => data ? Object.values(data.sections).reduce((sum, items) => sum + items.length, 0) : 0, [data]); const needsYou = data?.sections.waitingOnHuman.length ?? 0; const working = data?.sections.researchingNow.length ?? 0; const learned = data?.sections.recentlyLearned.length ?? 0
  ...
  return <section style={{ marginBottom: 32 }}>
    ...
    {failed && !data ? <div style={{ ...glass(0.035), borderRadius: 14, padding: '16px 18px', fontSize: 11.5, color: ROSE }}>I could not load live activity. I will keep trying.</div> : <div style={{ ...glass(0.025), borderRadius: 16, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '190px minmax(0, 1fr)', minHeight: 250 }}><div style={{ padding: 8, borderRight: '1px solid rgba(255,255,255,0.06)' }}>{SECTIONS.map((section) => { const count = data?.sections[section.key].length ?? 0; const active = activeSection === section.key; return <button key={section.key} type="button" onClick={() => setActiveSection(section.key)} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, border: 0, borderRadius: 9, background: active ? 'rgba(255,255,255,0.055)' : 'transparent', padding: '9px 10px', color: active ? TEXT : TEXT_MUTED, cursor: 'pointer', textAlign: 'left' }}><span style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}><span aria-hidden style={{ width: 6, height: 6, borderRadius: '50%', flexShrink: 0, background: count ? section.color : TEXT_QUIET }} /><span style={{ fontSize: 11.5, whiteSpace: 'nowrap' }}>{section.shortTitle}</span></span><span style={{ fontSize: 9.5, color: active ? section.color : TEXT_QUIET }}>{count}</span></button> })}</div>
        <div style={{ minWidth: 0, padding: '12px 16px 10px' }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 3 }}><div style={{ fontSize: 11, fontWeight: 700, color: TEXT }}>{activeConfig.title}</div><div style={{ fontSize: 9.5, color: TEXT_QUIET }}>{activeItems.length} item{activeItems.length === 1 ? '' : 's'}</div></div>{activeItems.length === 0 ? <div style={{ padding: '24px 2px', fontSize: 11.5, lineHeight: 1.5, color: TEXT_QUIET }}>{activeConfig.empty}</div> : <div>{activeItems.map((item, index) => <ActivityRow key={`${item.label}-${index}`} item={item} color={activeConfig.color} future={activeConfig.future} onOpen={() => setSelected({ section: activeConfig, item })} />)}</div>}</div></div>
