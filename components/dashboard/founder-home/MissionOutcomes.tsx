...
function MissionCard({ mission }: { mission: MissionOutcome }) {
  return (
    <article style={{ ...glass(0.04), borderRadius: 14, padding: '16px 17px', minWidth: 0 }}>
      <div style={{ fontSize: 10.5, fontWeight: 750, letterSpacing: '0.065em', color: mission.key === 'employment' ? AQUA : EMERALD }}>
        {mission.title}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(110px, 1fr))', gap: 8, marginTop: 13 }}>
        {mission.metrics.map((item) => (
          <div key={item.key} title={item.evidence} style={{ padding: '9px 10px', borderRadius: 10, background: 'rgba(255,255,255,0.025)' }}>
      ...
      <div style={{ marginTop: 13, paddingTop: 11, borderTop: '1px solid rgba(255,255,255,0.055)' }}>
        ...
        <div style={{ fontSize: 12, lineHeight: 1.5, color: TEXT, marginTop: 4 }}>
          {mission.bottleneck?.statement ?? 'Insufficient comparable funnel evidence to identify a bottleneck.'}
        </div>
...
export default function MissionOutcomes() {
  ...
  const load = useCallback(async () => {
    ...
    if (!session) return
    const response = await fetch('/api/founder/direction/outcomes', {
  ...
  if (error) {
    return <section style={{ marginBottom: 30, fontSize: 11.5, color: TEXT_MUTED }}>Outcome evidence is currently unavailable. Direction is not substituting cached or synthetic success metrics.</section>
  }
  if (!data) return null
  ...
  return (
    <section style={{ marginBottom: 30 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 12, marginBottom: 9 }}>
        <div>
          <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '0.055em', color: TEXT_QUIET }}>CURRENT OUTCOMES</div>
          <div style={{ fontSize: 11.5, color: TEXT_MUTED, marginTop: 3 }}>Computed live from canonical job-search, outreach, lifecycle, and commercial state.</div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: 10 }}>
        <MissionCard mission={data.employment} />
        <MissionCard mission={data.revenue} />
      </div>
    </section>
  )
}
